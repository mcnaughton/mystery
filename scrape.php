...
function handle_item($filename, $basedir = '') {
	...
	$cleaned = trim(str_replace( array("\x0D\x03", "\x0D", "\x03", "\xD2", "\xF5\xF5", "\xF5"), array("\n", "\n", "", '"', '"',"'"), $raw));
	$cleaned = str_replace( array("\xD0", "\xF2", "\xF3", "\xF4","`", "\xF5", "\xD2\xD5", "\xD5\xD5", "\xD5", "\xD3", "\xD1", "\xD4"), array('--', '"', '"', '"', "'", "'", '"', '"', "'", '"', '--', "'"), $cleaned );
	...
	if ( false === strpos($cleaned,"Date:")) {
		...
		foreach ( $content as $item ) {
			...
			if ( false === strpos($piece,"000") && 0 !== strpos( $piece, "0" )) {
	...
	#file_put_contents("results.json", json_encode($results), APPEND_FILE | LOCK_EX);
	#echo "yearly/$year/$month/$day/$md5.json\n";
}
